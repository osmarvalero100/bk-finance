    ...
    try:
        ...
        if category_dict.get("parent_id"):
            parent = (
                ...
                .filter(
                    and_(
                        Category.id == category_dict["parent_id"],
        ...
        db_category = Category(
            **category_dict, user_id=current_user.id, is_default=False
        )
        db.add(db_category)
        db.commit()
    ...
    try:
        ...
        if include_subcategories:
            ...
            for category in categories:
                ...
                category_dict.subcategories = _get_subcategories_recursive(
                    category.id, db, current_user.id
                )
...
async def update_category(
    ...
    try:
        ...
        if category_data.parent_id is not None:
            parent_id = (
                category_data.parent_id if category_data.parent_id != 0 else None
            )
            if parent_id:
                parent = (
                    db.query(Category)
...
async def delete_category(
    ...
    try:
        ...
        expenses_count = (
            db.query(Expense).filter(Expense.category_id == category_id).count()
        )
...
def _get_subcategories_recursive(
    parent_id: int, db: Session, user_id: int
) -> List[CategoryWithSubcategories]:
    """Función auxiliar para obtener subcategorías recursivamente"""
    subcategories = (
        db.query(Category)
        .filter(and_(Category.parent_id == parent_id, Category.user_id == user_id))
