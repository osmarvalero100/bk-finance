...
async def get_dashboard_summary(
    ...
    try:
        ...
        active_budgets_count = db.query(func.count(Budget.id)).filter(
        ...
        recent_expenses = db.query(Expense).options(joinedload(Expense.category)).filter(
            Expense.user_id == current_user.id
        ).order_by(desc(Expense.date)).limit(10).all()

        # Obtenemos los últimos 10 ingresos
        recent_incomes = db.query(Income).options(joinedload(Income.category)).filter(
            Income.user_id == current_user.id
        ).order_by(desc(Income.date)).limit(10).all()

        # Unificamos y ordenamos
        transactions = []
        for e in recent_expenses:
            transactions.append(DashboardTransaction(
                id=e.id,
