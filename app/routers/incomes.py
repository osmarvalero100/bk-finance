    ...
    try:
        ...
        if income_data.tag_ids:
            tags = (
                ...
                .filter(
                    and_(
                        Tag.id.in_(income_data.tag_ids),
        ...
        db_income = Income(
            **income_data.model_dump(exclude={"tag_ids"}), user_id=current_user.id
        )
        db.add(db_income)
...
async def get_incomes(
    ...
    try:
        query = (
            db.query(Income)
            .options(joinedload(Income.tags))
...
async def get_income(
    ...
    try:
        income = (
            db.query(Income)
            .options(joinedload(Income.tags))
            .filter(and_(Income.id == income_id, Income.user_id == current_user.id))
            .first()
        )
